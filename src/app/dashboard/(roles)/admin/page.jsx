  ...
  <Card sx={{ height: '100%', boxShadow: 2 }}>
    <CardContent>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
        <Box sx={{ p: 1, borderRadius: 1, bgcolor: `${color}.light`, color: `${color}.main`, mr: 2 }}>{icon}</Box>
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // 1. Usuarios Totales
        const { count: usuariosCount, error: usersError } = await supabase.from('profiles').select('*', { count: 'exact', head: true });
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} sm={6} md={4}>
          <StatCard title="Usuarios Totales" value={stats.usuarios} icon={<PeopleIcon />} color="primary" />
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
          <StatCard title="Solicitudes Activas" value={stats.solicitudes} icon={<DescriptionIcon />} color="warning" />
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
          <StatCard title="Monto Colocado" value={formatoMoneda(stats.montoColocado)} icon={<MonetizationOnIcon />} color="success" />
        </Grid>
