...
export default function ComingSoon({ chip, primaryBtn, heading, description, form, illustration }) {
  const [email, setEmail] = useState('');
  const [snackbar, setSnackbar] = useState(null);

  const submitUrl = form?.submitUrl || '/api/subscribe';
  const submitPayload = form?.submitPayload || null;
  const placeholder = form?.placeholder || 'Enter your email address';
  const successMessage = form?.successMessage || null;
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const response = await axios.post(submitUrl, { email, ...(submitPayload || {}) });
      if (response.status === 200 || response.status === 201) {
        const message = successMessage || response.data?.message || 'Gracias. Te contactaremos pronto.';
        setSnackbar({ open: true, message, severity: 'success' });
        setEmail('');
      } else {
        setSnackbar({ open: true, message: response.data?.error || 'No se pudo enviar. Intenta de nuevo.', severity: 'error' });
      }
    } catch (error) {
      const message =
      ...
      >
        ...
        <Stack component="form" onSubmit={submitHandler} sx={{ gap: { xs: 7, md: 12 }, alignItems: 'center' }}>
          <Stack sx={{ gap: { xs: 2, sm: 3 }, alignItems: 'center' }}>
            ...
            {illustration ? illustration : heading ? <Typography variant="h2" align="center">{heading}</Typography> : <Comingsoon />}
